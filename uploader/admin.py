                    ...
                    try:
                        with xlrd.open_workbook(temp.name) as wb:
                            ...
                            for i in range(len(sh.row(0))):
                                if str(sh.row(0)[i].value).lower() not in keys:
                                    keys[str(sh.row(0)[i].value).lower()] = i
                            for i in range(1, sh.nrows):
                                if 'сategory' in keys and sh.row(i)[keys['сategory']].value:
                                    ...
                                    automodel2 = None
                                    manufacturer = None
                                    if Category.objects.filter(name=sh.row(i)[keys['сategory']].value).exists():
                                        category = Category.objects.filter(name=sh.row(i)[keys['сategory']].value).first()
                                    else:
                                        category = Category.objects.create(name=sh.row(i)[keys['сategory']].value)
                                    if 'subcategory1' in keys and sh.row(i)[keys['subcategory1']].value and category:
                                        if Subcategory.objects.filter(name=sh.row(i)[keys['subcategory1']].value).exists():
                                            subcategory = Subcategory.objects.filter(name=sh.row(i)[keys['subcategory1']].value).first()
                                        else:
                                            subcategory = Subcategory.objects.create(name=sh.row(i)[keys['subcategory1']].value, category=category)
                                    if 'subcategory2' in keys and sh.row(i)[keys['subcategory2']].value and Subcategory:
                                        if Subcategory1.objects.filter(name=sh.row(i)[keys['subcategory2']].value).exists():
                                            subcategory1 = Subcategory1.objects.filter(name=sh.row(i)[keys['subcategory2']].value).first()
                                        else:
                                            subcategory1 = Subcategory1.objects.create(name=sh.row(i)[keys['subcategory2']].value, subcategory=subcategory)
                                    if 'autobrand' in keys and sh.row(i)[keys['autobrand']].value:
                                        if Brand.objects.filter(name=sh.row(i)[keys['autobrand']].value).exists():
                                            brand = Brand.objects.filter(name=sh.row(i)[keys['autobrand']].value).first()
                                        else:
                                            brand = Brand.objects.create(name=sh.row(i)[keys['autobrand']].value)
                                    if 'automodel1' in keys and sh.row(i)[keys['automodel1']].value and brand:
                                        if Automodel.objects.filter(name=sh.row(i)[keys['automodel1']].value).exists():
                                            automodel = Automodel.objects.filter(name=sh.row(i)[keys['automodel1']].value).first()
                                        else:
                                            automodel = Automodel.objects.create(name=sh.row(i)[keys['automodel1']].value, brand=brand)
                                    if 'automodel2' in keys and sh.row(i)[keys['automodel2']].value and automodel:
                                        if Automodel1.objects.filter(name=sh.row(i)[keys['automodel2']].value).exists():
                                            automodel1 = Automodel1.objects.filter(name=sh.row(i)[keys['automodel2']].value).first()
                                        else:
                                            automodel1 = Automodel1.objects.create(name=sh.row(i)[keys['automodel2']].value, automodel=automodel)
                                    if 'automodel3' in keys and sh.row(i)[keys['automodel3']].value and automodel1:
                                        if Automodel2.objects.filter(name=sh.row(i)[keys['automodel3']].value).exists():
                                            automodel2 = Automodel2.objects.filter(name=sh.row(i)[keys['automodel3']].value).first()
                                        else:
                                            automodel2 = Automodel2.objects.create(name=sh.row(i)[keys['automodel3']].value, automodel1=automodel1)
                                    if 'manufacturer' in keys and sh.row(i)[keys['manufacturer']].value:
                                        if Manufacturer.objects.filter(name=sh.row(i)[keys['manufacturer']].value).exists():
                                            manufacturer = Manufacturer.objects.filter(name=sh.row(i)[keys['manufacturer']].value).first()
                                        else:
                                            manufacturer = Manufacturer.objects.create(name=sh.row(i)[keys['manufacturer']].value)
                                    code = sh.row(i)[keys['productcode']].value if 'productcode' in keys else None
                                    name = sh.row(i)[keys['productname']].value if 'productname' in keys else None
                                    description = sh.row(i)[keys['shortdescription']].value if 'shortdescription' in keys else None
                                    price = sh.row(i)[keys['price']].value if 'price' in keys else None
                                    if name:
